...
class AestheticScore(db.Model):
    ...
    # 关联关系
    style = db.relationship('Style', backref='aesthetic_scores')
    image = db.relationship('Image', backref='aesthetic_scores')
    
    __table_args__ = (
        db.UniqueConstraint('style_id', 'image_id', 'evaluator_type', name='uk_style_image_evaluator'),
    )
